...
function lintManifest(manifest) {
	...
	if (Array.isArray(manifest.catalogs)) manifest.catalogs.forEach(function(catalog, i) {
		...
		if (catalog.hasOwnProperty('extraSupported') && !Array.isArray(catalog.extraSupported))
			errors.push(new Error('manifest.catalogs['+i+']: extraSupported must be an array'))

		if (catalog.hasOwnProperty('extraRequired') && !Array.isArray(catalog.extraRequired))
			errors.push(new Error('manifest.catalogs['+i+']: extraRequired must be an array'))
	})
